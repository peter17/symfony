 ...
 * bug #63599 [Validator] Fix required options check when extending a constraint with a simplified constructor (nicolas-grekas)
 * bug #63592 [Cache] Add timeout and slot eviction to LockRegistry stampede prevention (nicolas-grekas)
 * bug #63591 [Validator] Fix when constraint without expression language installed, when using closure expression (annadamm-check24)
 * bug #63589 [Form] Fix session data contamination by non-serializable objects in form flow (nicolas-grekas)
 ...
 * bug #63583 [DoctrineBridge] Rename `_schema_subscriber_check` table to `schema_subscriber_check_` for Oracle compatibility (moneire)
 * bug #63573 [HttpClient] Fix CachingHttpClient compatibility with decorator clients on 304 responses (nicolas-grekas)
 ...
 * bug #63563 [TypeInfo] Fix StringTypeResolver calling Type::enum() on interfaces extending BackedEnum (gnutix)
 * bug #63547 [Serializer] Fix nullable array constructor parameter overriding collection value type (nicolas-grekas)
 ...
 * bug #63542 [TypeInfo] Fix resolving class const type (gharlan)
 * bug #63510 [PropertyInfo][ReflectionExtractor] fix is/can/has type resolver on method without property (Guilain)
 * bug #63522 [Notifier] Fix `SweegoTransport` by allowing bool values (qdequippe)
 ...
 * bug #63508 [DependencyInjection] Handle `Stringable` for string-typed arguments in `CheckTypeDeclarationsPass` (yoeunes)
 * bug #63509 [JsonStreamer] Fix missing generator for shared types in self-referencing objects (mtarld)
